...
test("workspace keeps the requested three-pane information architecture", () => {
  ...
  assert.match(app, /<section class="detail-pane" aria-label="设备详情"/);
  assert.match(app, /type ResizePane = "sidebar" \| "list" \| "generator"/);
  assert.match(app, /const RESIZER_WIDTH = 8/);
  assert.match(app, /const GENERATOR_DEFAULT_WIDTH = 460/);
  assert.match(app, /const GENERATOR_MIN_WIDTH = 360/);
  assert.match(app, /const GENERATOR_MAX_WIDTH = 680/);
  ...
  assert.match(app, /window\.innerWidth - 120/);
...
test("top-level actions avoid fake window chrome and keep generator after add device", () => {
  ...
  assert.match(app, /<button class="primary-button" data-tooltip="新增设备"/);
  assert.match(app, /<button class="tool-button topbar-tool" data-tooltip="批量改密"/);
  assert.match(app, /<button class="tool-button topbar-tool accent" data-tooltip="密码生成器"/);
  assert.match(app, /class="tool-button" data-tooltip="新增账号"/);
  assert.match(app, /class="tool-button update-password-action" data-tooltip="更新密码"/);
  assert.match(app, /class="tool-button" data-tooltip="复制账号信息"/);
  assert.match(app, /class="tool-button" data-tooltip="编辑账号"/);
  assert.match(app, /class="icon-button" aria-label="更多操作" data-tooltip="更多操作"/);
  assert.match(styles, /button\[data-tooltip\]::before \{[\s\S]*content: attr\(data-tooltip\);[\s\S]*background: rgba\(34, 38, 42, 0\.96\);/);
  assert.match(styles, /button\[data-tooltip\]:hover::before,[\s\S]*button\[data-tooltip\]:focus-visible::after \{[\s\S]*opacity: 1;/);
  assert.match(styles, /\.topbar button\[data-tooltip\]::before,[\s\S]*\.detail-actions button\[data-tooltip\]::before,[\s\S]*\.modal-header button\[data-tooltip\]::before \{[\s\S]*top: calc\(100% \+ 10px\);/);
  assert.match(styles, /button\[data-tooltip\]::before,[\s\S]*button\[data-tooltip\]::after \{[\s\S]*left: 50%;/);
  assert.match(styles, /\.drawer-footer button\[data-tooltip\]::before \{[\s\S]*transform: translate\(-50%, 4px\);/);
  assert.doesNotMatch(styles, /\.pane-actions button\[data-tooltip\]::before \{[\s\S]*left: 0;/);
  assert.doesNotMatch(styles, /\.drawer-footer button\[data-tooltip\]::before,[\s\S]*\.topbar button\[data-tooltip\]::before,[\s\S]*left: auto;/);
  assert.doesNotMatch(styles, /\.drawer-footer button\[data-tooltip\]::after,[\s\S]*\.topbar button\[data-tooltip\]::after,[\s\S]*right: 14px;/);
});
...
test("status toast stays above the interface and can be dismissed", () => {
  ...
  assert.match(app, /function clearStatusTimer\(\)/);
  ...
  assert.match(app, /statusHovered = false;[\s\S]*scheduleStatusDismiss\(duration\)/);
  assert.match(app, /function dismissStatus\(\)/);
  assert.match(app, /function pauseStatusDismiss\(\)[\s\S]*statusHovered = true;[\s\S]*clearStatusTimer\(\)/);
  assert.match(app, /function resumeStatusDismiss\(\)[\s\S]*statusHovered = false;[\s\S]*scheduleStatusDismiss\(3000\)/);
  assert.match(app, /<div class="toast" role="status" on:pointerenter=\{pauseStatusDismiss\} on:pointerleave=\{resumeStatusDismiss\}>/);
...
test("bulk password updates target usernames and reuse generated passwords", () => {
  ...
  assert.match(app, /function getBulkPasswordMatchKey\(match: Pick<BulkPasswordMatch, "itemId" \| "accountId">\)/);
  assert.match(app, /!bulkPasswordDeselectedKeys\.includes\(getBulkPasswordMatchKey\(match\)\)/);
  assert.match(app, /function toggleBulkPasswordMatch\(match: BulkPasswordMatch\)/);
  assert.match(app, /function selectAllBulkPasswordMatches\(\)/);
  assert.match(app, /function clearBulkPasswordMatches\(\)/);
  assert.match(app, /\$: bulkPasswordSelectedMatches = bulkPasswordMatches\.filter/);
  assert.doesNotMatch(app, /bulkPasswordSelectedKeys|previousBulkPasswordCandidateKeys|syncBulkPasswordSelection/);
  ...
  assert.match(app, /canUseGeneratorForCurrentAccount = generatorTarget !== "bulk-password"/);
  assert.match(app, /canUseGeneratorForBulkUpdate = generatorTarget !== "current-account"/);
  assert.match(app, /if \(!canUseGeneratorForCurrentAccount\) return/);
  assert.match(app, /if \(!canUseGeneratorForBulkUpdate\) return/);
  assert.match(app, /\{#if canUseGeneratorForCurrentAccount\}/);
  assert.match(app, /\{#if canUseGeneratorForBulkUpdate\}/);
  ...
  assert.match(app, /function updateBulkPasswordUsername\(username: string\)/);
  assert.match(app, /if \(bulkPasswordForm\.username === username\) return/);
  assert.match(app, /bulkPasswordForm = \{ \.\.\.bulkPasswordForm, username \}/);
  assert.match(app, /on:input=\{\(event\) => updateBulkPasswordUsername\(\(event\.currentTarget as HTMLInputElement\)\.value\)\}/);
  assert.match(app, /指定用户名/);
  assert.match(app, /已选择 \{bulkPasswordSelectedMatches\.length\} \/ \{bulkPasswordMatches\.length\} 个账号/);
  assert.match(app, /类型：\$\{bulkPasswordForm\.deviceType\}/);
  assert.match(app, /aria-label="批量改密账号选择"/);
  assert.match(app, /on:click=\{selectAllBulkPasswordMatches\}>全选/);
  assert.match(app, /on:click=\{clearBulkPasswordMatches\}>清空/);
  assert.match(app, /checked=\{isBulkPasswordMatchSelected\(match\)\}/);
  assert.match(app, /on:change=\{\(\) => toggleBulkPasswordMatch\(match\)\}/);
  assert.match(app, /disabled=\{!bulkPasswordForm\.password\.trim\(\) \|\| bulkPasswordSelectedMatches\.length === 0\}/);
  assert.match(app, /已更新 \$\{matches\.length\} 个账号/);
  assert.match(app, /批量改密/);
  assert.match(styles, /\.bulk-preview/);
  assert.match(styles, /\.bulk-match-list/);
  assert.match(styles, /\.bulk-match-row/);
  assert.match(styles, /\.bulk-selection-actions/);
});
...
test("a device can hold multiple account credentials", () => {
  ...
  assert.match(app, /class="readonly-field" aria-label="所属设备"/);
  assert.match(app, /\{selectedItem\.deviceName \|\| "未选择设备"\}/);
  assert.doesNotMatch(app, /accountForm\.tag/);
  assert.doesNotMatch(app, /<input bind:value=\{accountForm\.tag\}/);
  assert.match(styles, /\.readonly-field/);
...
test("generator length is directly editable and clamped on commit", () => {
  ...
  assert.match(app, /class="drawer-resizer"/);
  assert.match(app, /aria-label="调整密码生成器宽度"/);
  assert.match(styles, /width:\s*min\(var\(--generator-width\), calc\(100vw - 88px\)\)/);
  assert.match(styles, /\.drawer-resizer/);
  assert.match(app, /class="drawer-action primary-action" aria-label="重新生成" data-tooltip="重新生成"/);
  assert.match(app, /class="drawer-action" aria-label="填入当前账号" data-tooltip="填入当前账号"/);
  assert.match(app, /class="drawer-action" aria-label="批量改密" data-tooltip="批量改密"/);
  assert.match(app, /RotateCcwKey size=\{20\}/);
  assert.doesNotMatch(app, /multi-key-icon/);
  assert.match(app, /class="drawer-action" aria-label="复制密码" data-tooltip="复制密码"/);
  assert.match(app, /class="icon-button inline" aria-label="重新生成" data-tooltip="重新生成"/);
  assert.match(app, /class="icon-button inline" aria-label="复制生成密码" data-tooltip="复制生成密码"/);
  assert.match(app.match(/<footer class="drawer-footer">[\s\S]*?<\/footer>/)?.[0] ?? "", /<span>重新生成<\/span>[\s\S]*<span>复制密码<\/span>/);
  assert.match(styles, /\.generator-drawer \{[\s\S]*container-type: inline-size;/);
  assert.match(styles, /\.drawer-footer \{[\s\S]*display: flex;[\s\S]*flex-wrap: nowrap;/);
  assert.match(styles, /\.drawer-action \{[\s\S]*flex: 1 1 0;[\s\S]*height: 44px;/);
  assert.match(styles, /\.drawer-action\.primary-action \{[\s\S]*background: var\(--blue\);/);
  assert.match(styles, /@container \(max-width: 430px\) \{[\s\S]*\.drawer-action \{[\s\S]*flex: 0 0 48px;[\s\S]*width: 48px;[\s\S]*\.drawer-action > span \{[\s\S]*display: none;/);
  assert.doesNotMatch(styles, /\.multi-key-icon/);
  assert.doesNotMatch(app, /当前字符池|generatorSummary|generatorStrengthLabel|getGeneratorStrengthLabel|class="generator-note"/);
  assert.doesNotMatch(styles, /\.generator-note/);
  assert.match(styles, /\.compact-action \{[\s\S]*width: 40px;[\s\S]*height: 40px;/);
  assert.match(styles, /\.icon-button\.inline \{[\s\S]*width: 36px;[\s\S]*height: 36px;/);
  assert.match(styles, /\.topbar \.primary-button,[\s\S]*\.detail-actions \.tool-button \{[\s\S]*width: 48px;[\s\S]*height: 48px;/);
  ...
  assert.match(app, /type="range"[\s\S]*min="3"[\s\S]*max="24"/);
  assert.match(app, /function updateGeneratorLengthFromSlider\(event: Event\)/);
  assert.match(app, /on:input=\{updateGeneratorLengthFromSlider\}/);
...
test("right-click context menus are available for types, devices, and details", () => {
  ...
  const typeBlankContextMenuMarkup = app.match(/\{:else if activePopover === "type-blank-context"\}[\s\S]*?\{:else if activePopover === "list-blank-context"/)?.[0] ?? "";
  const listBlankContextMenuMarkup = app.match(/\{:else if activePopover === "list-blank-context" \|\| activePopover === "detail-blank-context"\}[\s\S]*?\{:else if activePopover === "device-context"\}/)?.[0] ?? "";
  ...
  assert.match(app, /type DeviceTypeSortMode = "default" \| "nameAsc" \| "countDesc"/);
  assert.match(app, /let deviceTypeSortMode: DeviceTypeSortMode = "default"/);
  assert.match(app, /function setDeviceTypeSortMode\(mode: DeviceTypeSortMode\)/);
  assert.match(app, /function sortDeviceTypeOptions\(types: DeviceTypeMeta\[\], mode: DeviceTypeSortMode, vaultItems: VaultItem\[\]\)/);
  ...
  assert.match(app, /window\.removeEventListener\("pointerdown", handleGlobalPointerDown, true\)/);
  assert.match(app, /window\.removeEventListener\("contextmenu", handleGlobalContextMenu, true\)/);
  ...
  assert.match(app, /function handleGlobalPointerDown\(event: PointerEvent\)/);
  assert.match(app, /function handleGlobalContextMenu\(event: MouseEvent\)/);
  assert.doesNotMatch(app, /menu-scrim|redispatchPointerEventAfterPopoverClose|handleMenuScrimContextMenu/);
  assert.doesNotMatch(styles, /\.menu-scrim/);
  ...
  assert.match(app, /<section class="detail-pane" aria-label="设备详情" on:contextmenu=\{openDetailBlankContextMenu\}>/);
  ...
  assert.doesNotMatch(app, /aria-label="关闭菜单"/);
  ...
  assert.match(app, />管理设备分类</);
  assert.match(typeBlankContextMenuMarkup, />新增设备类型</);
  assert.match(typeBlankContextMenuMarkup, />默认顺序</);
  assert.match(typeBlankContextMenuMarkup, />类型名称 A-Z</);
  assert.match(typeBlankContextMenuMarkup, />设备数量优先</);
  assert.doesNotMatch(typeBlankContextMenuMarkup, /排序设备类型/);
  assert.match(app, /"当前密码库范围"/);
  assert.match(app, /"未选择设备"/);
  assert.match(app, /disabled=\{deviceTypeOptions\.length === 0\}/);
  assert.match(app, /title=\{deviceTypeOptions\.length === 0 \? "请先在左栏创建设备类型" : "新增设备"\}/);
  assert.match(listBlankContextMenuMarkup, />新增设备</);
  assert.match(listBlankContextMenuMarkup, />最近更新优先</);
  assert.match(listBlankContextMenuMarkup, />设备名称 A-Z</);
  assert.match(listBlankContextMenuMarkup, />设备类型 A-Z</);
  assert.doesNotMatch(listBlankContextMenuMarkup, /<span>新增设备类型<\/span>|类型名称 A-Z|设备数量优先|默认顺序/);
