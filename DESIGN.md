...
## Product goals
- Goals: 本地保存真实设备资产和账号密码；记录密码更新历史；按设备名称、IP、资产编号和设备位置快速查找；快速生成随机密码；支持明文查看和复制；支持 JSON、CSV、YAML 配置导入导出。
...
- Config files: JSON、CSV、YAML 导出都使用中文字段名。JSON 和 YAML 共享设备类型、设备、账号、密码历史的中文层级结构，YAML 主要用于人工阅读和手动编辑；CSV 使用单表格式，第一行是中文表头，下面按设备类型、设备名称、用户名一条账号一行展示，密码历史作为数组写入单元格。YAML 导出统一使用 `.yaml`，导入接受 `.yaml` 和 `.yml`。导入会整体覆盖当前资产库，但执行覆盖前必须拒绝同一设备类型下的同名设备、同一设备下的同名账号以及缺失的必填名称，并在界面中显示具体原因。导入只接受当前中文配置格式，不再兼容旧版英文 JSON、旧版分段 CSV 或 INI。JSON、CSV 和 YAML 都是包含账号密码及密码历史的明文配置，不提供加密保护。
