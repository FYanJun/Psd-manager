...
## Product goals
- Goals: 本地保存真实设备资产和账号密码；记录密码更新历史；按设备名称和 IP 快速查找；快速生成随机密码；支持明文查看和复制；支持 JSON、CSV、INI 配置导入导出。
...
- Boundary rules: IP、设备名称、设备类型和设备备注属于设备；用户名、密码、账号标签和密码历史属于账号；右键菜单必须跟随当前区域语义，不能把账号操作混到设备区域。
...
- Config files: JSON、CSV、INI 导出都使用中文字段名，并统一按设备类型分组。结构顺序为元信息、设备类型总表、隐藏设备类型、各设备类型下的设备、设备下的账号、账号下的密码历史。导入只接受当前中文分组配置格式，不再兼容旧版英文 JSON、旧版单表 CSV 或旧版 `[account.*]` INI。

## Content voice
- Tone: 简短、工具化、中文为主。
- Terminology: 使用“设备”表示真实设备或资产；使用“账号”表示某台设备下的一组用户名和密码；使用“设备类型”区分路由器、NAS、服务器等；使用“密码历史”表示旧密码记录。
- Microcopy rules: 按钮用明确动词，如“新增设备”“编辑设备”“新增账号”“复制密码”“生成密码”；字段名保持短词，如“用户名”“密码”“IP 地址”“最后编辑”；避免“用户信息”等容易和设备/账号边界混淆的文案。
