...
Psd Manager 是一个轻量级设备资产凭据管理工具，用来集中记录真实设备资产、账号密码、密码历史和批量改密记录。应用采用 Rust + Tauri + Svelte 构建，数据默认保存在本机。
...
```bash
xattr -dr com.apple.quarantine "/Applications/设备资产凭据管理工具.app"
```
...
- 导出的配置文件名会带日期和时间，例如 `设备资产凭据配置-2026-06-12-15-42.json`，便于保留同一天的多份配置。
- 导出时可以选择 JSON、CSV、INI。JSON 会完整保留应用结构；CSV 和 INI 以账号配置为单位，适合人工查看和简单编辑。
- 导入支持 JSON、CSV、INI。导入前会显示结构化摘要，包括设备、账号、密码历史、设备类型、格式版本和导出时间。
- 导入配置会覆盖当前资产库；如需保留现有数据，请先导出当前配置。
- JSON 导入兼容旧版配置文件，包括早期根节点为设备数组的 JSON 文件。
